...
#            io_index = all_dict_list["pcu_io_indexes"][("%s_INDEX") % (io)]
            if (io in list(all_dict_list["pcuioreg_scope"].keys())):
                if (all_dict_list["pcuioreg_scope"][io] not in list(CFG["PCUIO_RTL_NODE_Name"].keys())):
                ...
                path = ("%s.%s_%s[0][0]") % (all_dict_list["pcuioreg_scope"][io], CFG["PCUIO_RTL_NODE_Name"][all_dict_list["pcuioreg_scope"][io]], io.upper())
